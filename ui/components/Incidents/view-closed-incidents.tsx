...
export function ViewClosedIncidents() {
...
//   const [userRole, setUserRole] = useState("employee") // Default to employee, will be updated from API
  ...
  // Fetch closed incidents when component mounts
  useEffect(() => {
    fetchClosedIncidents(currentPage, pageSize)
  }, [currentPage, pageSize])



  const fetchClosedIncidents = async (page: number, size: number) => {
    setLoading(true)
    try {
      // Modify this to use your actual API
      const response = await incidentAPI.getClosedIncidentsFiltered({
        page,
        pageSize: size,
      })
      ...
      console.error("Failed to fetch closed incidents:", error)
      toast.error("Failed to load closed incidents")
    } finally {
      setLoading(false)
    }
  }
  ...
  const handlePageChange = (newPage: number) => {
    if (newPage !== currentPage) {
      fetchClosedIncidents(newPage, pageSize)
    }
  }

  // Page size change handler
  const handlePageSizeChange = (newPageSize: number) => {
    fetchClosedIncidents(1, newPageSize) // Reset to first page
  }
  ...
  const filteredAndSortedIncidents = incidents
    .filter((incident) => {
      ...
      // Severity filter
      if (filters.severity && incident.severityLevel !== filters.severity) {
        return false
      }
    ...
    <div className="px-4 sm:px-6 lg:px-8 bg-gray-50 dark:bg-gray-900 min-h-screen">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div className="sm:flex-auto">
          ...
          <h1 className="text-2xl font-semibold text-gray-900 mt-2">Closed Incidents</h1>
          <p className="mt-2 text-sm text-gray-700">
            A list of all closed incidents in your organization including their reference number, type, severity, and
            resolution details.
          </p>
        </div>
      </div>

      <InfoPanel title="Closed Incidents Archive" icon={<ArchiveIcon className="h-5 w-5 text-blue-600" />}>
        <p className="text-sm">
          This page displays all incidents that have been marked as closed. Closed incidents have been fully resolved
          and documented. You can search and filter through the archive to find specific incidents for reference or
          reporting purposes.
        </p>
      </InfoPanel>
        ...
        <Card className="mt-8 border-2 border-dotted border-red-300">
          <CardContent className="flex flex-col items-center justify-center p-8">
            ...
            <p className="text-gray-500 text-center max-w-md">
              {incidents.length === 0
                ? "There are no closed incidents in the system."
                : "No closed incidents match your current filter criteria."}
            </p>
      ...
      <Sheet open={!!selectedIncident} onOpenChange={() => setSelectedIncident(null)}>
        <SheetContent className="w-full sm:max-w-xl overflow-y-auto max-h-screen bg-white dark:bg-gray-800">
          <SheetHeader>
            <SheetTitle>Closed Incident Details</SheetTitle>
          </SheetHeader>
